...
function selectMenu(type) {
	var oprov = document.getElementById("provinceId");
	var ocity = document.getElementById("cityId");
	//var oarea = document.getElementById("areaId");
	var oprovArr = oprov.options;
	var ocityArr = ocity.options;

	//oarea.options.length = 1;	

	if(oprov.selectedIndex == 0) {
		ocity.options.length = 1;
		return;
	}
	for(var i = 0; i < citys.length; i++) {
		for(var j = 0; j < citys[i].city.length; j++) {
			if(type > 0 && oprovArr[oprov.selectedIndex].value == citys[i].name) {	
				if(j == 0) {
					ocity.options.length = 1;
				}
				var oopt = document.createElement("option");	//添加城市
				oopt.innerHTML = citys[i].city[j].name;
				oopt.value = citys[i].city[j].name;
				ocity.appendChild(oopt);
			} else if(type < 0 && ocityArr[ocity.selectedIndex].value == citys[i].city[j].name) {
				for(var k = 0; k < citys[i]. city[j].area.length; k++) {
					var oopt = document.createElement("option");	//添加区域
					oopt.innerHTML = citys[i].city[j].area[k];
					oarea.appendChild(oopt);
				}
				break;
			}
		}
...
function initProvince() {
	...
	for(var i = 0; i < citys.length; i++) {
		...
		oopt.innerHTML = citys[i].name;
		oopt.value = citys[i].name;
